...
class JournalService {
  ...
  public addJournalEntry(data: Omit<JournalEntry, 'id' | 'date'> & { id?: string; date?: string }): JournalEntry {
    const now = new Date();
    ...
    const entry: JournalEntry = {
      ...
      price: data.price,
      amount: data.amount,
      fee: data.fee || parseFloat((data.price * data.amount * 0.00075).toFixed(4)),
      pnl: data.pnl || 0,
      pnlPercent: data.pnlPercent || 0,
      mlProbability: data.mlProbability || 75,
      modelName: data.modelName || 'XGBoost Classifier',
      entryReason: data.entryReason || 'RSI Oversold + EMA Crossover',
  ...
  private generateInitialSeedSnapshots(): DailySnapshot[] {
    ...
    for (let dayOffset = 10; dayOffset >= 0; dayOffset--) {
      ...
      const dailyPnL = 45 + ((10 - dayOffset) * 22) - (dayOffset % 2 === 0 ? 15 : -30);
      currentEquity += dailyPnL;

      snapshots.push({
        date: dateStr,
        equity: parseFloat(currentEquity.toFixed(2)),
        realizedPnL: parseFloat(dailyPnL.toFixed(2)),
        unrealizedPnL: parseFloat((Math.sin(dayOffset) * 25).toFixed(2)),
        winRate: parseFloat((65 + (dayOffset % 15)).toFixed(1)),
        totalTrades: 3 + (dayOffset % 3),
        bestModel: 'XGBoost Classifier',
        bestStrategy: 'RSI Oversold + EMA Crossover',
        timestamp: new Date(d.getFullYear(), d.getMonth(), d.getDate(), 23, 59).toISOString()
      });
    }
